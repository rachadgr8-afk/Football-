...
export default function App() {
  ...
  const [activePlayerUrl, setActivePlayerUrl] = useState<string>('/videos/football_match.mp4');
  const [activePlayerPoster, setActivePlayerPoster] = useState<string>('/videos/poster_10s.jpg');
  ...
  useEffect(() => {
    fetch('https://fotbal-1.onrender.com/api/presets')
      .then((res) => res.json())
      .then((data) => {
        if (data.presets && data.presets.length > 0) {
          setPresets(data.presets);
          setSelectedVideo(data.presets[0]);
          setActivePlayerUrl(data.presets[0].sourceUrl);
          setActivePlayerPoster(data.presets[0].posterUrl || '/videos/poster_10s.jpg');
  ...
  // Handle custom video upload to real server
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setTestStatus(`Uploading ${file.name} to video engine...`);
    ...
    try {
      ...
      const uploadedMeta = {
        ...
        description: 'User uploaded real football footage',
        duration: 75.0,
        sourceUrl: data.videoUrl,
        localPath: data.localPath,
        tags: ['Custom Upload', 'User Footage'],
        defaultSubject: 'Striker / Player',
      };

      setSelectedVideo(uploadedMeta);
      setActivePlayerUrl(data.videoUrl);
      setActivePlayerTitle(`Uploaded: ${file.name}`);
      setUploadedFileName(file.name);
      setIsMasterRendered(false);
      setTestStatus(`Uploaded successfully! Ready for real FFmpeg processing.`);
    } catch (err: any) {
  ...
  const runTest1 = async () => {
    ...
    try {
      ...
      setActivePlayerUrl(data.videoUrl);
      if (data.posterUrl) setActivePlayerPoster(data.posterUrl);
  ...
  const runTest3 = async () => {
    setIsTestRunning(true);
    setTestStatus('Executing TEST 3: FFmpeg drawtext "TEST CINEMATIC" from 2s -> 4s...');

    try {
      const res = await fetch('/api/test-render-3', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          localPath: selectedVideo?.localPath || 'public/videos/football_match.mp4',
  ...
  const runCompleteRealPipeline = async (customStyle?: AIStyle) => {
    ...
    try {
      ...
      const renderRes = await fetch('/api/render-full-cinematic', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          localPath: selectedVideo.localPath || 'public/videos/football_match.mp4',
      ...
      setActivePlayerUrl(renderData.videoUrl);
      if (renderData.posterUrl) setActivePlayerPoster(renderData.posterUrl);
  ...
  const handleOpenVideoInNewTab = () => {
    window.open(activePlayerUrl, '_blank');
  };

  const handleSaveToDevice = () => {
    const a = document.createElement('a');
    a.href = activePlayerUrl;
  ...
  const handleShare = () => {
    if (navigator.share) {
      navigator.share({
        ...
        url: window.location.origin + activePlayerUrl,
      });
    } else {
      navigator.clipboard.writeText(window.location.origin + activePlayerUrl);
  ...
  const handleReset = () => {
    if (presets.length > 0) {
      ...
      setActivePlayerUrl(presets[0].sourceUrl);
                  ...
                  return (
                    <button
                      ...
                      onClick={() => {
                        ...
                        setActivePlayerUrl(preset.sourceUrl);
                        setActivePlayerPoster(preset.posterUrl || '/videos/poster_10s.jpg');
